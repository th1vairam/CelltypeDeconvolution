...
all.results = plyr::ldply(1:dim(exp.m)[2], .fun = function(i, X, Y, nperm){
  ...
  modCIBERSORT <- function(X, y){
    ...
    # w[pval<=0.05] = 0
    return(list("w" = cbind(w, pval), "mix_rmse" = rmse, "mix_r" = corr))
  }
...
all.results1 = all.results %>%
  ...
  tidyr::separate(celltype, c('celltype1','pval'),sep = '\\.') %>%
  dplyr::filter(is.na(pval))
p = ggplot(all.results1, aes(x = BrainRegion, y = value, color = Diagnosis)) + geom_boxplot() + facet_grid(celltype1~.)
...
all.results1 %>%
  dplyr::select(SampleID, celltype1, value) %>%
  tidyr::spread(celltype1, value) %>%
  write.table(file = 'deconvFrac.tsv', sep = '\t', row.names = F, quote = F)
obj = File('deconvFrac.tsv', name = 'Deconvoluted Fractions', parentId = CODE$properties$id)
obj = synStore(obj, activityName = activityName, used = as.character(all.used.ids), executed = thisFile)
```