...
gene.markers = intersect(gene.markers.mayo, gene.markers.mssm$Gene)
...
for (ngenes in 1:4){
  ...
  exp.r = exp[rownames(exp) %in% gl,  covariates$SampleID]
...
```{r cibersort.new, fig.height=10, fig.width=10}
cb.results = list()
pl1 = list()
for (ngenes in 1:3){
  if (ngenes == 1)
    gl = gene.markers.mayo
  else if (ngenes == 2)
    gl = gene.markers.mssm$Gene
  else
    gl = gene.markers
  gl = unique(gl)
  ...
  p = ggplot(all.results, aes(x = CellType, y = value, color = Study)) + geom_boxplot() + facet_grid(celltype~.)
  p = p + theme(axis.text.x = element_text(angle = 45, hjust = 1)) + ggtitle(paste(c('MAYO','MSSM','INTERSECTION')[ngenes],'Gene List'))
  p = p + xlab('Original Label') + ylab('Estimated Fraction') + theme(legend.position = 'top')
  pl1[[ngenes]] = p
  
  cb.results[[ngenes]] = all.results
}
multiplot(plotlist = pl1, cols = 3)
```
...
for (ngenes in 1:3){
  if (ngenes == 1)
    ...
    gl = gene.markers.mssm %>%
      dplyr::rename(Symbol = Gene, CellType = Module) %>%
      dplyr::mutate(CellType = factor(CellType, labels =  c("ast" = "Astrocyte", "end" = "Endothelial", "mic" = "Microglia","neu" = "Neuron","oli" = "Oligodendrocytes", "opc" = "OPC")))
  else
    gl = dplyr::filter(gene.markers.mssm, Gene %in% gene.markers) %>%
      dplyr::rename(Symbol = Gene, CellType = Module) %>%
      dplyr::mutate(CellType = factor(CellType, labels =  c("ast" = "Astrocyte", "end" = "Endothelial", "mic" = "Microglia","neu" = "Neuron","oli" = "Oligodendrocytes", "opc" = "OPC")))
      
  gl = unique(gl) %>%
    dplyr::filter(Symbol %in% expr$hgnc_symbol)

  exp = data.matrix(expr[,-(1)])
  rownames(exp) = expr[,1]
  
  exp.m = exp[,  covariates$SampleID]
  
  model = DSA(exp.m, gl[,c('Symbol', 'CellType')])
  w = (t(model$est.weight)/rowSums(t(model$est.weight))) %>%
    rownameToFirstColumn('SampleID') %>%
    tidyr::gather(celltype, fraction, -SampleID)
