---
title: "Cell type deconvolution of reprocessed RNASeq from AMP-AD (unadjusted - Most Expressed)"
...
knit2synapse::knitToFolderEntity(file = "./ampad_repr_unadj_deconvolution_MostExp.Rmd",
                                 parentId = "syn9890608",
                                 entityName = "AMPAD Reprocessed RNASeq (With Most Expressed Genes)")
```
...
thisFileName <- 'ampad_repr_unadj_deconvolution_MostExp.Rmd'
...
gl.new = plyr::dlply(gl, .(CellType), .fun = function(x, expr){
  tmp = apply(expr[x$Symbol,],1,median, na.rm = T)
  tmp = data.frame(Symbol = names(tmp), md = tmp) %>%
    dplyr::arrange(desc(md))
  tmp$Symbol[which(tmp$md >= median(tmp$md)+sd(tmp$md))] %>%
    as.character()
}, expr)
```
...
CODE = Folder(name = 'AMPAD Reprocessed RNASeq (With Most Expressed Genes)', parentId = parentId)
CODE = synStore(CODE)
