...
gl.new = plyr::llply(unique(covariates$BrainRegion), .fun = function(region, expr, covariates, gl){
  plyr::dlply(gl, .(CellType), .fun = function(x, expr){
    datExpr = (expr[as.character(x$Symbol),covariates$SampleID[covariates$Diagnosis=='AD' & covariates$BrainRegion == region]])
    ...
    datExpr1 = (expr[as.character(x$Symbol),covariates$SampleID[covariates$Diagnosis=='CONTROL' & covariates$BrainRegion == region]])
...
gl.new1 = list()
for(i in 1:6){
  gl.new1[[i]] = Reduce(c, lapply(gl.new, function(x){(x[[i]])})) %>% unique
}
names(gl.new1) = names(gl.new[[1]])
gl.new = gl.new1
```
