...
knit2synapse::knitToFolderEntity(file = "./ampad_repr_unadj_deconvolution_cibersort.Rmd",
                                 parentId = "syn9890608",
                                 entityName = "Cell type fractions - AMPAD - with lcpm signatures - using cibersort")
```
...
cibersort.estimated.fractions = cibersort.estimated.fractions %>%
  tidyr::gather(celltype, fraction, -contains('.pval'), -nBrainRegions, -SampleID, -mix_rmse, -mix_r) %>%
  tidyr::gather(celltype.pval, pval, -celltype, -fraction, -nBrainRegions, -SampleID, -mix_rmse, -mix_r) %>%
  dplyr::select(-celltype.pval) %>%
  dplyr::rename(fdr = pval)

ggplot(cibersort.estimated.fractions, aes(x = nBrainRegions, y = fraction, color = celltype)) + geom_boxplot() + facet_grid(.~celltype) + theme(legend.position = 'top') + ggtitle('Cell type fractions')
ggplot(cibersort.estimated.fractions, aes(x = nBrainRegions, y = fdr, color = celltype)) + geom_boxplot() + facet_grid(.~celltype) + theme(legend.position = 'top') + ggtitle('FDR')

tmp1 = cibersort.estimated.fractions %>%
  dplyr::select(nBrainRegions, celltype, fraction, fdr) %>%
  dplyr::filter(nBrainRegions == 'any1') %>%
  dplyr::select(-nBrainRegions)
tmp2 = cibersort.estimated.fractions %>%
  dplyr::select(nBrainRegions, celltype, fraction, fdr) %>%
  dplyr::filter(nBrainRegions == 'any5') %>%
  dplyr::select(-nBrainRegions) %>%
  dplyr::rename(fraction1 = fraction, fdr1 = fdr)
tmp = cbind(tmp1, tmp2)
```
```{r comp1, fig.height=6, fig.width=12}
p = ggplot(tmp, aes(x = fraction, y = fraction1, color = celltype)) + geom_point() 
p = p + geom_smooth(method = 'loess') + facet_grid(.~celltype) + geom_abline(slope=1)
...
write.table(cibersort.estimated.fractions, file = 'deconvFracLcpm.tsv', sep = '\t', row.names = F, quote = F)
